...
def generate_launch_description():
    ...
    # Named-waypoint and saved-mission persistence for any client (this
    # app, a future web API) — see app_data_store_node.py. Unconditional
    # and mode-independent like system_monitor above: bookmarks/missions
    # are relevant whether the robot is idle, mapping, or navigating, so
    # this isn't tied to start_nav/start_slam the way docking is.
    app_data_store = Node(
        package='navpromini_controller',
        executable='app_data_store_node',
        name='navpromini_app_data_store',
        output='screen',
        parameters=[{'use_sim_time': use_sim_time}],
    )
