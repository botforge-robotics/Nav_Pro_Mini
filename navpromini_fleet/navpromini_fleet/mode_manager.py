...
Listens to:
  ...
  fleet_cmd    (std_msgs/String JSON)  — start_nav | stop_nav | factory_reset | set_pose
...
class ModeManager(Node):
    def __init__(self) -> None:
        ...
        self._level_id: str = ''
        self._map_name: str = 'active'
        self._pending_pose: Optional[dict[str, float]] = None
        self._pose_flush_left = 0

        self._status_pub = self.create_publisher(String, 'mapping_status', 10)
    ...
    def _stop_slam(self) -> None:
        self._stop_proc(self._slam_proc, 'slam')
        self._slam_proc = None

    def _stop_nav(self) -> None:
        self._stop_proc(self._nav_proc, 'nav')
        self._nav_proc = None
    ...
    def _cmd_start_slam(self, level_id: str) -> None:
        if self._mode == 'MAPPING' and self._slam_proc and self._slam_proc.poll() is None:
            self._publish_mapping_status('mapping')
            return
        self._stop_nav()
        self._stop_slam()
        ...
        self._publish_mapping_status('mapping', levelId=level_id)
    ...
    def _cmd_stop_slam(self) -> None:
        ...
        self.get_logger().info('SLAM stopped → HARDWARE')
    ...
    def _on_fleet_cmd(self, msg: String) -> None:
        ...
        with self._lock:
            if action in ('start_nav', 'nav_start', 'start'):
                ...
                self._publish_initial_pose(payload)
            else:
                self.get_logger().warn(f'unknown fleet action: {action}')

    def _cmd_start_nav(self, payload: dict[str, Any]) -> None:
        self._stop_slam()
        map_name = str(payload.get('mapName') or payload.get('map_name') or self._map_name or 'active')
        maps = self._ensure_maps_dir()
        level_id = str(payload.get('levelId') or payload.get('level_id') or self._level_id or '')

        # Claim occupancy from fleet server when levelId present
        if level_id:
            try:
                from navpromini_fleet.map_claim import claim_map
                cfg = load_fleet_config(Path(str(self.get_parameter('config_path').value)))
                if cfg is not None and cfg.provisioning_token:
                    claim_map(level_id, maps, cfg.api_base, cfg.provisioning_token, map_name)
                    self.get_logger().info(f'claimed map for level {level_id} as {map_name}')
            except Exception as exc:  # noqa: BLE001
