...
from nav2_mission_planner_interfaces.srv import DeleteMap, GetMapList, LaunchWithArgs
...
class MapsHandler(BaseHandler):
    ...
    async def post(self) -> None:
        """Save the current map under a name.
        ...
        resp = await call_service(self.bridge.cli_launch, req, 'save_map', timeout=120.0)

        already = 'already exist' in (resp.message or '').lower()
        if resp.success:
            self.send({'saved': True, 'name': name}, status=201)
        elif already and data.get('overwrite'):
            # The save ran; only the "is this new?" check failed.
            self.send({'saved': True, 'name': name, 'overwritten': True})
        elif already:
...
class ActivateMapHandler(BaseHandler):
    ...
    async def post(self, name: str) -> None:
        ...
        result = await switch_mode(self, 'navigation', name)
